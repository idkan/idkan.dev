...
export default function Post ({ postData, prevPost, nextPost, props }) {
  const { slug, fileName, tags, title } = props
  return (
    <>
      <div className='divide-y divide-gray-200 dark:divide-gray-700 xl:col-span-3 xl:row-span-2 xl:pb-0'>
        <div className='prose max-w-none pt-10 pb-8 dark:prose-dark'>{postData}</div>
        <div className='pt-6 pb-6 text-sm text-gray-700 dark:text-gray-300 flex items-center gap-2'>
          <Link href={shareOnTwitter(title, `${siteMetadata.siteUrl}/blog/${slug}`)} className='hover:underline'>
            Discuss on Twitter
          </Link>
          {' • '}
          <Link href={editUrl(fileName)} className='flex items-center hover:underline'>Did you find a bug? Edit it on Github
            <svg viewBox='0 0 24 24' className='w-4 h-4 ml-2' fill='currentColor'>
